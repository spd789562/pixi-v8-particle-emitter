...
import type { ValueList } from './PropertyNode';
  ...
  if (config.acceleration && (config.acceleration.x || config.acceleration.y)) {
    ...
    if (config.speed && 'start' in config.speed) {
      ...
      minStart =
        ...
        ((config as EmitterConfigV2).minimumSpeedMultiplier ?? 1);
    ...
    if (config.speed) {
      if ('start' in config.speed) {
        ...
        out.behaviors.push({
          ...
          config: {
            speed: config.speed,
            minMult: (config as EmitterConfigV2).minimumSpeedMultiplier ?? 1,
          },
        });
      }
    }
  ...
  if (config.scale) {
    if ('start' in config.scale) {
      ...
      out.behaviors.push({
        ...
        config: {
          scale: config.scale,
          minMult: (config as EmitterConfigV2).minimumScaleMultiplier ?? 1,
        },
      });
    }
  }
